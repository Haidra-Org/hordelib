...
_anima_turbo_base_model_name = "Anima-Turbo-v1.1"
...
@pytest.fixture(scope="session")
def anima_turbo_base_model_name(shared_model_manager: type[SharedModelManager]) -> str:
    """The default Anima Turbo model name used for testing once its pending record is published."""
    assert shared_model_manager.manager.compvis is not None
    if _anima_turbo_base_model_name not in shared_model_manager.manager.compvis.model_reference:
        pytest.skip(f"{_anima_turbo_base_model_name} is not (yet) in the horde model reference")
    return _anima_turbo_base_model_name
...
def pytest_collection_modifyitems(items):
    ...
    MODULES_TO_RUN_FIRST = [
        ...
        "tests.test_cuda",
        "tests.test_utils",
        "tests.installation.test_manifest",
        "tests.installation.test_installer",
        "tests.test_comfy",
