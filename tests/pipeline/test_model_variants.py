...
the two qwen-family models, which differ in every one of those variables.
"""
...
def _materialize(model_name: str, extra_files: dict[str, str], **payload_overrides: Any):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (QWEN, KREA2_MODEL_NAME, 2.0, (None, 2.0)),
        (QWEN, KREA2_MODEL_NAME, None, (None, None)),
...
def test_krea2_runs_the_qwen_graph_with_its_own_encoder_and_no_shift():
    definition, graph = _materialize(KREA2_MODEL_NAME, KREA2_FILES)
...
def test_a_requested_shift_is_ignored_for_krea2():
    _, graph = _materialize(KREA2_MODEL_NAME, KREA2_FILES, flow_shift=2.0)
