...
class TestRegistryCoverage:
    def test_every_known_baseline_has_an_entry(self) -> None:
        ...
        missing = [
            ...
            # "infer" is a directive, not a runnable baseline; flux_1 is the canonical flux value.
    ...
    def test_distinct_baselines_have_distinct_burden_seeds(self) -> None:
        """Graph sharing does not collapse baseline-specific resource accounting."""
        krea2 = estimate_job_burden(baseline="krea2_turbo", width=1024, height=1024)
        qwen = estimate_job_burden(baseline="qwen_image", width=1024, height=1024)
        assert krea2.vram_mb != qwen.vram_mb

    @pytest.mark.parametrize(
        ("baseline", "model_name"),
        [("qwen_image", None), ("z_image_turbo", None), ("krea2_turbo", None), ("anima", None)],
    )
    ...
    @pytest.mark.parametrize(
        "baseline",
        ["flux_1", "flux_schnell", "flux_dev", "stable_diffusion_xl", "krea2_turbo", "anima"],
    )
